...
POINT_CLOUD_REGISTER_POINT_STRUCT(ThunderPointXYZIRT,
    (float, x, x)
    (float, y, y)
    (float, z, z)
...
private:
    ...
    pcl::PointCloud<ThunderPointXYZIRT>::Ptr tmpThunderCloudIn;
...
public:
    ...
    void allocateMemory()
    {
        ...
        tmpThunderCloudIn.reset(new pcl::PointCloud<ThunderPointXYZIRT>());
    ...
    bool cachePointCloud(const sensor_msgs::msg::PointCloud2::SharedPtr& laserCloudMsg)
    {
        ...
        else if (sensor == SensorType::THUNDER) {
            // Convert to Velodyne format
            pcl::moveFromROSMsg(currentCloudMsg, *tmpThunderCloudIn);
            laserCloudIn->points.resize(tmpThunderCloudIn->size());
            laserCloudIn->is_dense = tmpThunderCloudIn->is_dense;
            //double time_begin = tmpPandarCloudIn->points[0].timestamp;
            for (size_t i = 0; i < tmpThunderCloudIn->size(); i++) {
                auto &src = tmpThunderCloudIn->points[i];
                auto &dst = laserCloudIn->points[i];
                dst.x = src.y * -1;
                dst.y = src.x;
                // dst.x = src.x;
    ...
    void projectPointCloud()
    {
        ...
        for (int i = 0; i < cloudSize; ++i)
        {
            ...
            int columnIdn = -1;
            if (sensor == SensorType::VELODYNE || sensor == SensorType::OUSTER || sensor == SensorType::THUNDER)
            {
